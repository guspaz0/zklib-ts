...
export class ZTCP {
  ...
  createSocket(cbError: (err: Error) => void, cbClose: (type: string) => void) {
  ...
  async requestData(msg: Buffer): Promise<Buffer> {
    try {
      return await new Promise((resolve, reject) => {
        ...
        const internalCallback = (data_1: Buffer) => {
          if (this.socket) {
        ...
        // Handle incoming data
        const handleOnData = (data_3: Buffer) => {
  ...
  async executeCmd(
    ...
    const callback = (responseData: Buffer) => {
   ...
   */
  readWithBuffer(
    reqData: Buffer | string,
    cb?: (received: number, total: number) => void,
  ): Promise<Record<string, Buffer | number | Error>> {
            ...
            const data = await new Promise((resolve2, reject2) => {
              try {
                this.sendChunkRequest(
                  i * Constants.MAX_CHUNK,
                  i === numberChunks ? remain : Constants.MAX_CHUNK,
                );
                this.socket?.on("data", (reply: Buffer) => {
                  ...
                  timer = setTimeout(() => {
                    internalCallback(
                      this.replyData,
                      new Error(
                        `TIME OUT !! ${this.packetNumber} PACKETS REMAIN !`,
                      ),
                    );
                  }, this.timeout);
                    ...
                    if (COMMANDS[headers.commandId]) {
                      switch (headers.commandId) {
                        ...
                        case COMMANDS.CMD_PREPARE_DATA:
                          ...
                          this.verbose &&
                            console.log(
                              `recieve chunk: prepare data size is ${headers.payloadSize}`,
                            );
                          break;
                        default:
                          break;
  ...
  async disableDevice() {
    try {
      const resp = await this.executeCmd(
  ...
  #listeners = new Map<number, (data: Buffer) => void>();
  ...
  async clearData(value?: number) {
    try {
      ...
      if (!value) value = 3;
      const buf = await this.executeCmd(
        COMMANDS.CMD_CLEAR_DATA,
        value.toString(),
      );
   ...
   */
  async getTemplates(
    callbackInProcess: any = () => {},
  ): Promise<Record<string, Finger[]>> {
  ...
  testTcpTop(packet) {
    ...
    if (
      headerValue1 === Constants.MACHINE_PREPARE_DATA_1 &&
  ...
  async sendWithBuffer(buffer: Buffer) {
    ...
    try {
      const cmdResponse = await this.executeCmd(
    ...
    try {
      for (let i = 0; i < packets; i++) {
        const resp = await this.sendChunk(
          buffer.slice(start, start + MAX_CHUNK),
        );
        if (resp) {
          start += MAX_CHUNK;
          if (i == packets - 1 && remain) {
            const lastPacket = await this.sendChunk(
              buffer.slice(start, start + remain),
            );
            return lastPacket;
          }
        }
  ...
  async readSocket(length: number, cb = null): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      function onDataEnroll(data) {
        ...
        replyBufer = Buffer.concat(
          [replyBufer, data],
          replyBufer.length + data.length,
        );
